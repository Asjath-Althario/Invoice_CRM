...
import { getCompanyProfile, mockProfitLoss } from '../../data/mockData';
...
const convertToCSV = () => {
    let csv = 'Profit & Loss\n\n';
    csv += 'Income\n';
    mockProfitLoss.income.forEach(i => csv += `"${i.label}",${i.amount}\n`);
    csv += '\nExpenses\n';
    mockProfitLoss.expenses.forEach(e => csv += `"${e.label}",${e.amount}\n`);
    return csv;
};
...
const ProfitLoss: React.FC = () => {
    ...
    const totalIncome = mockProfitLoss.income.reduce((sum, item) => sum + (item.amount || 0), 0);
    const totalExpenses = mockProfitLoss.expenses.reduce((sum, item) => sum + (item.amount || 0), 0);
            ...
            <div className="overflow-x-auto border dark:border-gray-700 rounded-lg max-w-3xl mx-auto">
                <table className="min-w-full bg-white dark:bg-gray-800">
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-600">
                        <ReportSection title="Income" items={mockProfitLoss.income} total={totalIncome} />
                        <ReportSection title="Expenses" items={mockProfitLoss.expenses} total={totalExpenses} />
                        <tr>
