...
import { getInvoices, getContacts, getQuotes, getPreferences, updatePreferences } from '../data/mockData';
...
const Dashboard: React.FC = () => {
    ...
    const calculateData = () => {
        ...
        const salesThisMonth = invoices
            .filter(inv => new Date(inv.issueDate) >= firstDayOfMonth)
        ...
        setRecentActivities(combinedActivities.slice(0, 5) as (Invoice | Quote)[]);
        ...
        invoices.filter(inv => inv.status === 'Paid').forEach(inv => {
            ...
            if(monthlySales.hasOwnProperty(monthKey)) {
                monthlySales[monthKey] += inv.total;
            }
        });
    ...
    const handleSavePreferences = (newDashboardWidgets: Preferences['dashboardWidgets']) => {
        const currentPrefs = getPreferences();
        updatePreferences({ ...currentPrefs, dashboardWidgets: newDashboardWidgets });
    };
