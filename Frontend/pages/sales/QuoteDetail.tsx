...
import { getQuotes, getContacts, addQuote, updateQuote, getProductsServices, addInvoice, getPreferences } from '../../data/mockData';
...
const QuoteDetail: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (quoteId) {
            const existingQuote = getQuotes().find(q => q.id === quoteId);
            if (existingQuote) {
    ...
    const handleConvertToInvoice = () => {
        ...
        const newInvoice = addInvoice({
            ...
            items: quote.items,
            status: 'Draft',
            comments: `Based on Quote #${quote.quoteNumber}`,
            subtotal: quote.subtotal || 0,
            tax: quote.tax || 0,
            total: quote.total || 0,
        });
        
        updateQuote({ ...quote as Quote, status: 'Converted' });
