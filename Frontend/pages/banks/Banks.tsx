...
import { Plus, Edit, Trash2, X, Landmark, Search, Wallet } from 'lucide-react';
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                <h1 className="text-3xl font-bold text-dark dark:text-light">Bank & Cash Accounts</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredAccounts.map(account => {
                    ...
                    const Icon = isCashAccount ? Wallet : Landmark;
                        ...
                        <div key={account.id} className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md flex flex-col justify-between">
                            ...
                            <div className="mt-6 flex items-center justify-between">
                                 ...
                                 <div className="flex space-x-2">
                                    {!isCashAccount && (
                                        <>
                                            <button onClick={() => setSelectedAccount(account)} className="text-primary hover:text-primary/80 dark:text-blue-400 dark:hover:text-blue-300"><Edit size={18}/></button>
                                            <button onClick={() => handleDeleteAccount(account.id)} className="text-red-600 hover:text-red-800 dark:text-red-500 dark:hover:text-red-400"><Trash2 size={18}/></button>
                                        </>
                                    )}
                                 </div>
                            </div>
